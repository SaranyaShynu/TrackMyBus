...
export default function AdminPanel() {
  ...
  const handleAddStaff = async (e) => {
    ...
    try {
      ...
      alert(`Success! ${staffForm.name} is now assigned.`);
  ...
  const handleUpdateUser = async (e) => {
    ...
    try {
      const config = { headers: { Authorization: `Bearer ${token}` } };
      // Note: Make sure the backend route is actually /api/admin/user/:id
  ...
  const openUserEdit = (user) => {
    setEditingUser({
      id: user._id, // Set ID correctly to avoid 404
      name: user.name,
      email: user.email,
    ...
    <div className={`min-h-screen flex flex-col transition-colors duration-300 ${theme}`}>
              ...
              <div className={`rounded-[2.5rem] border overflow-hidden ${card}`}>
                <table className="w-full text-left">
                  <thead className={adminDark ? 'bg-slate-800/50' : 'bg-slate-100'}>
                    <tr>
                      <th className="p-6 text-[10px] font-black uppercase text-slate-500 tracking-widest">Plate No</th>
                      <th className="p-6 text-[10px] font-black uppercase text-slate-500 tracking-widest">Route</th>
              ...
              <form onSubmit={handleAddStaff} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="md:col-span-2 space-y-2">
                  ...
                  <select value={staffForm.assignedBus} onChange={e => setStaffForm({ ...staffForm, assignedBus: e.target.value })} className={`w-full p-5 rounded-2xl border-2 font-bold outline-none ${input}`}>
                    ...
                    {buses.map(b => <option key={b._id} value={b._id}>{b.busNo} | Route: {b.route}</option>)}
                  </select>
                </div>
                  ...
                  <tbody className="divide-y divide-slate-500/10">
                    {filteredUsers.map((u) => (
                      <tr key={u._id} className="hover:bg-slate-500/5 transition-colors group">
                        ...
                        <td className="p-6">
                          <span className={`px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-tighter ${u.role === 'admin' ? 'bg-purple-500/10 text-purple-500' : 'bg-blue-500/10 text-blue-500'}`}>{u.role}</span>
                        </td>
                        <td className="p-6">
                          {u.assignedBus ? <div className="flex flex-col"><span className="font-bold text-xs">{u.assignedBus.busNo}</span><span className="text-[9px] opacity-40 uppercase font-bold">{u.assignedBus.schoolBuilding || 'Main'}</span></div> : <span className="text-[10px] font-black uppercase text-slate-500/40 italic">Not Assigned</span>}
                        </td>
                        <td className="p-6">
                          <div className="flex justify-end items-center gap-2">
                            <button onClick={() => openUserEdit(u)} className="p-2.5 rounded-xl bg-amber-500/10 text-amber-500 hover:bg-amber-500 hover:text-slate-950 transition-all"><Edit2 size={16} /></button>
                            <button onClick={() => handleDeleteUser(u._id)} className="p-2.5 rounded-xl bg-red-500/10 text-red-500 hover:bg-red-500 hover:text-white transition-all"><Trash2 size={16} /></button>
                          </div>
                        </td>
                      </tr>
                    ))}
      ...
      {showModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/50 backdrop-blur-sm p-4">
          <div className={`w-full max-w-md p-8 rounded-[2rem] border overflow-y-auto max-h-[90vh] ${adminDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200 shadow-2xl'}`}>
            <h2 className="text-xl font-black mb-6 uppercase italic tracking-tighter">
              Manage <span className="text-amber-500">{showModal === 'user' ? editingUser.name : editingBus.busNo}</span>
            </h2>
            ...
            <form onSubmit={showModal === 'user' ? handleUpdateUser : handleUpdateBus} className="space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <select value={editingUser.assignedBus} onChange={e => setEditingUser({ ...editingUser, assignedBus: e.target.value })} className={`w-full p-5 rounded-2xl border-2 font-bold outline-none ${adminDark ? 'bg-slate-800 border-transparent text-white' : 'bg-slate-100 border-slate-200'}`}>
                      ...
                      {buses.map(b => <option key={b._id} value={b._id}>{b.busNo} ({b.schoolBuilding})</option>)}
                    </select>
                  </div>
                </div>
              ) : (
              ...
              <div className="flex gap-3">
                ...
                <button type="button" onClick={() => setShowModal(null)} className="px-6 py-4 rounded-xl font-black uppercase tracking-widest text-[10px] opacity-50">Cancel</button>
              </div>
            </form>
          </div>
        </div>
